...
testSet <- na.omit(data.frame(na.omit(assa[1:21,8]), shift(na.omit(assa[1:21,8]), 1 ), shift(na.omit(assa[1:21,8]), 2 ), shift(na.omit(assa[1:21,8]), 3 ) ))
testSet <- testSet[nrow(testSet),]


xals <- data.frame( testTemp[2,22] ,  (testTemp[2,21]), (testTemp[2,20]) , (testTemp[2,19]) )
colnames(testSet) <- c("nolag","lag1","lag2","lag3")
colnames(xals) <- colnames(testSet)
testSet <- rbind(testSet ,  xals)

xals <- data.frame( testTemp[2,23] ,  (testTemp[2,22]), (testTemp[2,21]) , (testTemp[2,20]) )

colnames(xals) <- colnames(testSet)
testSet <- rbind(testSet ,  xals)

xals <- data.frame( testTemp[2,24] ,  (testTemp[2,23]), (testTemp[2,23]) , (testTemp[2,21]) )
...
plot(testTemp[2,],type="b",col="blue", ylim=c(0,max(testTemp[2,],testTemp[8,])))
lines(testTemp[8,],type="b",col="red")
lines(c(testTemp[8,0:(ncol(testTemp)-3)], predsvm[3:4]),type="b",col="green")
