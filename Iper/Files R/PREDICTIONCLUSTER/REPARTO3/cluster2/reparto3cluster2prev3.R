...
testSet <- na.omit(data.frame(na.omit(assa[,6]), shift(na.omit(assa[,6]), -1 ), shift(na.omit(assa[,6]), -2 )))
# testSet <- rbind(testSet, na.omit(assa[,7]))
colnames(testSet) <- c("nolag","lag1","lag2")
predictFrame <- (c (as.vector(as.matrix(assa[,(1:3)])) , na.omit(as.vector(as.matrix(assa[,6])))))
