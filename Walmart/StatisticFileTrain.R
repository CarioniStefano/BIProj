...
HolidayDates <- data.frame(Date=time(HolidayDates), HolidayDates, check.names=FALSE, row.names=NULL)
...
app <- data.frame(Date=time(store1.z), store1.z, check.names=FALSE, row.names=NULL)
app[,1] <- as.numeric(app[,1])

app2 <- data.frame(Date=time(store2.z), store2.z, check.names=FALSE, row.names=NULL)
app2[,1] <- as.numeric(app2[,1])

app3 <- data.frame(Date=time(store3.z), store3.z, check.names=FALSE, row.names=NULL)
app3[,1] <- as.numeric(app3[,1])

app4 <- data.frame(Date=time(store4.z), store4.z, check.names=FALSE, row.names=NULL)
app4[,1] <- as.numeric(app4[,1])

app5 <- data.frame(Date=time(store5.z), store5.z, check.names=FALSE, row.names=NULL)
app5[,1] <- as.numeric(app5[,1])

app6 <- data.frame(Date=time(store6.z), store6.z, check.names=FALSE, row.names=NULL)
...
moreStores <- merge(moreStores, app3 , by=c("Date","Dept"),all=TRUE)
moreStores <- merge(moreStores, app4 , by=c("Date","Dept"),all=TRUE)
moreStores <- merge(moreStores, app5 , by=c("Date","Dept"),all=TRUE)
moreStores <- merge(moreStores, app6 , by=c("Date","Dept"),all=TRUE)
moreStores <- moreStores [ order(moreStores[,2]), ]
colnames(moreStores) <- c("Date","Dept","IsHoliday","Store1","WeeklySalesStore1","Store2","WeeklySalesStore2","Store3","WeeklySalesStore3","Store4","WeeklySalesStore4","Store5","WeeklySalesStore5","Store6","WeeklySalesStore6")
...
storeData <- data.frame(Store=rep(NA, rowsNumber),Department=rep(NA, rowsNumber),media=rep(NA, rowsNumber), varianza=rep(NA, rowsNumber), standardDeviation=rep(NA, rowsNumber), skewnesss=rep(NA, rowsNumber), curtosi=rep(NA, rowsNumber), quant1=rep(NA, rowsNumber), quant5=rep(NA, rowsNumber),  # as many cols as you need
...
for (j in c(1:6)) {
  ...
  for (i in sort(unique(moreStores[,2]))) {
    ...
    print(mean(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)])))
    print(var(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)])))
    print(sd(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)])))
    print(skewness(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)])))
    print(kurtosis(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)])))
    print(quantile(moreStores[which(moreStores$Dept==i),3+(j*2)],probs=c(0.01,0.05), na.rm=TRUE))
    
    
    mean <-mean(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)]))
    var <- var(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)]))
    sd <- (sd(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)])))
    skew <- skewness(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)]))
    kurt <-kurtosis(na.omit(moreStores[which(moreStores$Dept==i),3+(j*2)]))
    quant <- quantile(moreStores[which(moreStores$Dept==i),3+(j*2)],probs=c(0.01,0.05), na.rm=TRUE)
    
    deptIndex <- c(i)
    deptIndexName <- "DEPT"
    
    storeData[((j-1)*99)+i, ] <- c(j,i,mean,var,sd,skew,kurt,unname(quant))
...
deptSelected <- moreStores[which(moreStores$Dept==21 | moreStores$Dept==81 | moreStores$Dept==91 | moreStores$Dept==40 | moreStores$Dept==7 | moreStores$Dept==92),]

write.csv(deptSelected,file="~/Lavoro/OutputFiles/deptSelected.csv", row.names = FALSE,append=FALSE)
...
namedepts <- c("Books an Magazine","Commercial Bread","Frozen Foods","Pharmacy OTC","Toys","Dry Grocery")
...
for (j in c(1:6)) {
  ...
  deptSelectedtemp [ ,(  3+(j*2) )] <- scale(deptSelectedtemp[ ,(  3+(j*2) )])[,1]
  ...
  colors <- c("coral1","chartreuse3","chocolate4","orange","black","purple")
  colIndex <- 1
  linetype <- 1
  pchDot <- 16
  ...
  for(i in ndepts){
    ...
    lines(tempDept$Date,tempDept[ ,(  3+(j*2) )] , lwd=2,
          lty=linetype, col=colors[colIndex], pch=pchDot)
  ...
  legend(xrange[1], yrange[2], namedepts, cex=1, col=colors,
         pch=pchDot, lty=linetype, title="Dept")
...
colnames(cor.mat)=c("IsHoliday","Store1","Store2","Store3","Store4","Store5","Store6")
rownames(cor.mat)=c("IsHoliday","Store1","Store2","Store3","Store4","Store5","Store6")
print(cor.mat)

ScatterCC.ret.mat=cbind(dept21[,1],dept21[,2],dept21[,3],dept21[,4],dept21[,5],dept21[,6],dept21[,7])
colnames(ScatterCC.ret.mat)=c("IsHoliday","Store1","Store2","Store3","Store4","Store5","Store6")
pairs(ScatterCC.ret.mat,pch=18,col="blue",cex=1.5,cex.axis=1.5)
...
StoresCorrelation <- cbind(StoresCorrelation, aggregate(moreStores$WeeklySalesStore1, by=list(Category1=moreStores$Date,Store1=moreStores$Store1), FUN=sum))
StoresCorrelation <- cbind(StoresCorrelation, aggregate(moreStores$WeeklySalesStore2, by=list(Category2=moreStores$Date,Store2=moreStores$Store2), FUN=sum))
StoresCorrelation <- cbind(StoresCorrelation, aggregate(moreStores$WeeklySalesStore3, by=list(Category3=moreStores$Date,Store3=moreStores$Store3), FUN=sum))
StoresCorrelation <- cbind(StoresCorrelation, aggregate(moreStores$WeeklySalesStore4, by=list(Category4=moreStores$Date,Store4=moreStores$Store4), FUN=sum))
StoresCorrelation <- cbind(StoresCorrelation, aggregate(moreStores$WeeklySalesStore5, by=list(Category5=moreStores$Date,Store5=moreStores$Store5), FUN=sum))
StoresCorrelation <- cbind(StoresCorrelation, aggregate(moreStores$WeeklySalesStore6, by=list(Category6=moreStores$Date,Store6=moreStores$Store6), FUN=sum))
...
colnames(StoresCorrelation) = c("Date","IsHoliday","Store1Cumulative","Store2Cumulative","Store3Cumulative","Store4Cumulative","Store5Cumulative","Store6Cumulative")

cor.mat=cor(cbind(StoresCorrelation[,2],StoresCorrelation[,3],StoresCorrelation[,4],StoresCorrelation[,5],StoresCorrelation[,6],StoresCorrelation[,7],StoresCorrelation[,8]))
colnames(cor.mat)=c("IsHoliday","Store1","Store2","Store3","Store4","Store5","Store6")
rownames(cor.mat)=c("IsHoliday","Store1","Store2","Store3","Store4","Store5","Store6")
print(cor.mat)

ScatterCC.ret.mat=cbind(StoresCorrelation[,2],StoresCorrelation[,3],StoresCorrelation[,4],StoresCorrelation[,5],StoresCorrelation[,6],StoresCorrelation[,7],StoresCorrelation[,8])
colnames(ScatterCC.ret.mat)=c("IsHoliday","Store1","Store2","Store3","Store4","Store5","Store6")
pairs(ScatterCC.ret.mat,pch=18,col="blue",cex=1.5,cex.axis=1.5)
