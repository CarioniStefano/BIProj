  ...
  for(settoreSelected in unique(appoggioDepts2017[which(appoggioDepts2017$REPARTO == repartoSelected),]$SETTORE)){
    ...
    for(gruppoSelected in unique(appoggioDepts2017[which(appoggioDepts2017$REPARTO == repartoSelected & appoggioDepts2017$SETTORE == settoreSelected),]$GRUPPO)){
      ...
      for(clusterCurrent in clusterList){
        ...
        # MI SALVO I DATI DELLA GERARCHIA PER SALVARE NEL POSTO GIUSTO PIù AVANTI (FAMIGLIA E ENTE, IL RESTO è NEI CICLI)
        ...
        # C'è ALMENO UNA COLONNA 2017 PER QUESTA GERARCHIA E CLUSTER? DEPRECATED
        if(length( colnames(dataModel)[colSums(is.na(dataModel)) > 0] ) != 0){
          
          training <- data.frame(1:26)
          
          # SELEZIONO COLONNE DI TRAINING
          for(colAssa in colnames(dataModel)[colSums(is.na(dataModel)) == 0]){
            
            
            training<- cbind(training, as.matrix(dataModel[1:26,colAssa])  )
          ...
          if(nrow(training) > 1){
            ...
            test <- data.frame(1:26)
            testNames <- data.frame(1:2)
            
            # SELEZIONO COLONNE DI TEST
            for(colAssa in colnames(dataModel)[colSums(is.na(dataModel)) > 0]){
              
              test<- cbind(test, as.matrix(dataModel[1:26,colAssa])  )
            ...
            training <- data.frame(1:27)
            
            # SELEZIONO COLONNE DI TRAINING
            for(colAssa in colnames(dataModel)[colSums(is.na(dataModel)) == 0]){
              
              
              training<- cbind(training, as.matrix(dataModel[1:27,colAssa])  )
            ...
            training <- data.frame(1:28)
            
            # SELEZIONO COLONNE DI TRAINING
            for(colAssa in colnames(dataModel)[colSums(is.na(dataModel)) == 0]){
              
              training<- cbind(training, as.matrix(dataModel[1:28,colAssa])  )
            ...
            effectiveValue <- t(data.frame(1:28))
            
            for(colAssa in colnames(dataModel)[colSums(is.na(dataModel)) > 0]){
              
              effectiveValue <- rbind(effectiveValue,t(na.omit(data.frame(dataModel[1:28,colAssa])) ) )
            ...
            ##############FINE PREVISIONI EFFETTIVE 
            
            if(raddoppiata){
              
              ###SE HO RADDOPPIATO LE RIGHE IN PRECEDENZA (SE C'ERA UNA SOLA RIGA DI TEST, UN SOLO 2017 IN QUEL CLUSTER) FACCIO IN MODO DI FARLE TORNARE A SINGOLA PER METTERLA POI NEL RISULTATO FINALE
...
# TOTAL TEST CONTIENE TUTTE LE PREVISIONI PER TUTTE LE GERARCHIE PER TUTTI I CLUSTER DEL 2017
totalTest <- unique(totalTest)
# RIMUOVO I DOPPIONI E METTO TUTTO IN APPOGIODEPTSPRED
...
appoggioDeptsPred[, 8:35] <- sapply(appoggioDeptsPred[, 8:35], as.numeric)

# METTO IN APPOGGIODEPTS2017REAL TUTTI I VALORI EFFETTIVI DEL 2017 PER LE STESSE GERARCHIE E CLUSTER
appoggioDepts2017real <- appoggioDepts2017[which(as.numeric(as.character(appoggioDepts2017$ANNONO)) == 2017 ),]

# BLABLA PUò ESSERE RIMPIAZZATO CON APPOGGIODEPTS2017REAL SENZA PROBLEMI(?)

###################### RIMUOVO DA ENTRAMBI I DATa frame le righe per cui non ho fatto la previsione
...
# CALCOLO MAPE PER TERZULTIMA COLONNA == PRIMA PREVISIONE

assss <- (abs( ( blabla[,(ncol(blabla)-1)] - appoggioDeptsPred[,(ncol(appoggioDeptsPred)-1)]) / blabla[,(ncol(blabla)-1)]) )

mean(head(sort(assss),-28))

rmse(actual = blabla[,(ncol(blabla)-1):ncol(blabla)] , predicted = appoggioDeptsPred[,(ncol(appoggioDeptsPred)-1):ncol(appoggioDeptsPred)] )
