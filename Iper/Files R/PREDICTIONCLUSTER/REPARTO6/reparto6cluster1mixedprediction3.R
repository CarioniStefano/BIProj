...
testSet <- na.omit(data.frame(na.omit(assa[1:21,9]), shift(na.omit(assa[1:21,9]), 1 ), shift(na.omit(assa[1:21,9]), 2 ), shift(na.omit(assa[1:21,9]), 3 ) ))
testSet <- testSet[nrow(testSet),]


xals <- data.frame( testTemp[3,22] ,  (testTemp[3,21]), (testTemp[3,20]) , (testTemp[3,19]) )
colnames(testSet) <- c("nolag","lag1","lag2","lag3")
colnames(xals) <- colnames(testSet)
testSet <- rbind(testSet ,  xals)

xals <- data.frame( testTemp[3,23] ,  (testTemp[3,22]), (testTemp[3,21]) , (testTemp[3,20]) )

colnames(xals) <- colnames(testSet)
testSet <- rbind(testSet ,  xals)

xals <- data.frame( testTemp[3,24] ,  (testTemp[3,23]), (testTemp[3,23]) , (testTemp[3,21]) )
...
plot(testTemp[3,],type="b",col="blue", ylim=c(0,max(testTemp[3,],testTemp[9,])))
lines(testTemp[9,],type="b",col="red")
lines(c(testTemp[9,0:(ncol(testTemp)-3)], predsvm[3:4]),type="b",col="green")
