...
testSet <- na.omit(data.frame(na.omit(assa[1:21,12]), shift(na.omit(assa[1:21,12]), 1 ), shift(na.omit(assa[1:21,12]), 2 ), shift(na.omit(assa[1:21,12]), 3 ) ))
testSet <- testSet[nrow(testSet),]


xals <- data.frame( testTemp[6,22] ,  (testTemp[6,21]), (testTemp[6,20]) , (testTemp[6,19]) )
colnames(testSet) <- c("nolag","lag1","lag2","lag3")
colnames(xals) <- colnames(testSet)
testSet <- rbind(testSet ,  xals)

xals <- data.frame( testTemp[6,23] ,  (testTemp[6,22]), (testTemp[6,21]) , (testTemp[6,20]) )

colnames(xals) <- colnames(testSet)
testSet <- rbind(testSet ,  xals)

xals <- data.frame( testTemp[6,24] ,  (testTemp[6,23]), (testTemp[6,23]) , (testTemp[6,21]) )
...
plot(testTemp[6,],type="b",col="blue", ylim=c(0,max(testTemp[6,],testTemp[12,])))
lines(testTemp[12,],type="b",col="red")
lines(c(testTemp[12,0:(ncol(testTemp)-3)], predsvm[3:4]),type="b",col="green")
