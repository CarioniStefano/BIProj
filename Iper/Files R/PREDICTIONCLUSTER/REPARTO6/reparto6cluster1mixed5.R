...
testSet <- na.omit(data.frame(na.omit(assa[1:21,11]), shift(na.omit(assa[1:21,11]), 1 ), shift(na.omit(assa[1:21,11]), 2 ), shift(na.omit(assa[1:21,11]), 3 ) ))
testSet <- testSet[nrow(testSet),]


xals <- data.frame( testTemp[5,22] ,  (testTemp[5,21]), (testTemp[5,20]) , (testTemp[5,19]) )
colnames(testSet) <- c("nolag","lag1","lag2","lag3")
colnames(xals) <- colnames(testSet)
testSet <- rbind(testSet ,  xals)

xals <- data.frame( testTemp[5,23] ,  (testTemp[5,22]), (testTemp[5,21]) , (testTemp[5,20]) )

colnames(xals) <- colnames(testSet)
testSet <- rbind(testSet ,  xals)

xals <- data.frame( testTemp[5,24] ,  (testTemp[5,23]), (testTemp[5,23]) , (testTemp[5,21]) )
...
plot(testTemp[5,],type="b",col="blue", ylim=c(0,max(testTemp[5,],testTemp[11,])))
lines(testTemp[11,],type="b",col="red")
lines(c(testTemp[11,0:(ncol(testTemp)-3)], predsvm[3:4]),type="b",col="green")
